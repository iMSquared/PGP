...
class Settings():
    def __init__(self, params):    
        ...
        if params.model_type == 'preference':
            self.data_type = 'preference'
        else:    
            self.data_type = 'success'
        
        if params.data_size == '1000':
            if params.model_type == 'preference' or params.model_type == 'rank' or params.model_type == 'cvae':
                self.train_file = ('1000/sim/success',)
                self.test_file: tuple = ('1000/sim/success_eval',)
                self.train_neg_file: tuple = ('1000/sim/fail',)
                self.test_neg_file: tuple = ('1000/sim/fail_eval',)
            elif params.model_type == 'alphago':
                self.train_file = ('1000/exec/success', '1000/exec/fail',)
                self.test_file: tuple = ('1000/exec/success_eval', '1000/exec/fail_eval',)
                self.train_neg_file: tuple = ()
                self.test_neg_file: tuple = ()
            self.pref_data_file = '1000'
        elif params.data_size == '100':
            if params.model_type == 'preference' or params.model_type == 'rank' or params.model_type == 'cvae':
                self.train_file = ('100/sim/success',)
                self.test_file: tuple = ('100/sim/success_eval',)
                self.train_neg_file: tuple = ('100/sim/fail',)
                self.test_neg_file: tuple = ('100/sim/fail_eval',)
            elif params.model_type == 'alphago':
                self.train_file = ('100/exec/success', '100/exec/fail',)
                self.test_file: tuple = ('100/exec/success_eval', '100/exec/fail_eval',)
                self.train_neg_file: tuple = ()
                self.test_neg_file: tuple = ()
            self.pref_data_file = '100'
        elif params.data_size == '10':
            if params.model_type == 'preference' or params.model_type == 'rank' or params.model_type == 'cvae':
                self.train_file = ('10/sim/success',)
                self.test_file: tuple = ('10/sim/success_eval',)
                self.train_neg_file: tuple = ('10/sim/fail',)
                self.test_neg_file: tuple = ('10/sim/fail_eval',)
            elif params.model_type == 'alphago':
                self.train_file = ('10/exec/success', '10/exec/fail',)
                self.test_file: tuple = ('10/exec/success_eval', '10/exec/fail_eval',)
                self.train_neg_file: tuple = ()
                self.test_neg_file: tuple = ()
            self.pref_data_file = '10'
        # ============================================================================================================================= 
...
def main(params):
    ...
    if params.model_type == 'preference':
        config_planner['ucb_const'] = 10
    elif params.model_type == 'rank' or params.model_type == 'alphago' or params.model_type == 'cvae':
        config_planner['ucb_const'] = 10
    ...
    if (config.custom_loss and (config.loss_6 or config.loss_2)) or config.rank:
        neg_train_dataset = []
        neg_test_dataset = []
        for filename in train_neg_filename:
            neg_train_dataset += glob.glob(f'{dataset_path}/{filename}/*.pickle')
        for filename in test_neg_filename:
            neg_test_dataset += glob.glob(f'{dataset_path}/{filename}/*.pickle')
...
if __name__ == '__main__':
    ...
    parser.add_argument("--data_size", type=str, default="100")
