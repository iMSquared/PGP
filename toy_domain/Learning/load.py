...
class BatchMaker():
    ...
    def __call__(self, data):
        ...
        for traj in data:
            ...
            if self.config.q_value:
                a.append(np.asarray(traj['action'][1:i+1]).reshape(1, -1, 2))
            else:
...
def get_loader(config, dataset: Dict,
    ...
    if config.data_type == 'success':
        ...
        # load sub-datasets
        success_files = glob.glob(os.path.join(config.path, f'{config.pref_data_file}/sim/success/*')) if train \
            else glob.glob(os.path.join(config.path, f'{config.pref_data_file}/sim/success_eval/*'))
        success_dataset = LightDarkDataset(config, success_files, transform=transform)
        entire_files = glob.glob(os.path.join(config.path, f'{config.pref_data_file}/sim/success/*')) + glob.glob(os.path.join(config.path, f'{config.pref_data_file}/sim/fail/*')) if train \
            else glob.glob(os.path.join(config.path, f'{config.pref_data_file}/sim/success_eval/*')) + glob.glob(os.path.join(config.path, f'{config.pref_data_file}/sim/fail_eval/*'))
