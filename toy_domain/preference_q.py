...
class GuideQValueNet():
    ...
    def sample(self, history, goal):
        ...
        sampled_actions = self._next_action(history, num_sample=self.config.batch_size)
        sampled_actions_tensor = th.from_numpy(sampled_actions).to(dtype=th.float32, device=th.device(self.device)).unsqueeze(1)
        data['action'] = th.cat((data['action'], sampled_actions_tensor), dim=1)

        # predict the value of sampled next actions
        with th.no_grad():
            time_start = time.time()
            # if self.config.rank or (self.config.preference_loss and not self.config.preference_softmax):
            #     pred = self.model.inference_sigmoid(data).detach().item()
            # else:
            pred = self.model(data)
            prob = th.nn.functional.softmax(pred, dim=0).squeeze().cpu().numpy()
            # sampled_idx = np.random.choice(self.config.batch_size, p=prob)
            sampled_idx = 0
            next_action = np.squeeze(sampled_actions[sampled_idx])
            next_action = tuple(next_action)
            next_action_value = np.exp(pred[sampled_idx].squeeze().item())
            
            time_end = time.time()
            infer_time = time_end - time_start

        return next_action, next_action_value, infer_time
