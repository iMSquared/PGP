...
from value_with_config import SettingValue, GuideValueNet
...
class RewardModel(RewardModel):
    ...
    def sample(self, state, action, next_state):
        ...
        # |TODO| currently, get reward after update, can make same as state case(before update)?
            ...
            if self.valuenet is not None:
                if self.is_goal_particles(state):
                    return 1
                else:
                    return 0
            else: 
...
class PolicyModel(RandomRollout):
    def _NextAction(self, state, r_range=(0,2), theta_range=(0, 2*math.pi)):
...
def main(config, save_dir=False):
    plotting = None
    save_log = False
    save_data = False
    save_sim_data = False
    # save_sim_data = 'sim_4.3_8'
    # name_dataset = 'exec_4.3_8'
    # name_dataset = None
    exp = save_dir
    ...
    num_planning = config['num_plan']

    if save_data:
        save_dir = os.path.join('/home/share_folder/dataset', '4.3/exec')
        if not os.path.exists(save_dir):
            os.mkdir(save_dir)
            
    if save_sim_data:
        save_dir_sim = os.path.join('/home/share_folder/dataset', '4.3/sim')
    ...
    if guide_value:
        nn_config_value = SettingValue(config)
        valuenet = GuideValueNet(nn_config_value, guide_value_const)
    else:
        valuenet = None

    for num_particles in config['num_sim']:
        num_sucess = 0
        num_fail = 0
        for n in range(num_planning):
            ...
            planner = POMCPOW(pomdp=light_dark_problem, max_depth=planning_horizon, planning_time=-1., num_sims=num_particles,
                discount_factor=discont_factor, save_dir_sim=save_dir_sim, exploration_const=config['ucb_const'],
            ...
            #                 num_visits_init=0, value_init=0)
            # planner = POMCPOW(pomdp=light_dark_problem, max_depth=planning_horizon, planning_time=-1., num_sims=num_particles,
            #                 discount_factor=discont_factor, save_dir_sim=save_dir_sim, exploration_const=120,
            #                 num_visits_init=0, value_init=0)
            # planner = POMCPOW(pomdp=light_dark_problem, max_depth=planning_horizon, planning_time=-1., num_sims=num_particles,
        ...
        if exp:
            with open(os.path.join(log_dir, 'result.txt'), 'a') as f:
                f.write(f"=============== #sim/exec: {num_particles} ===============\n")
                success_rate = 100 * num_sucess / (num_sucess + num_fail)
                f.write("success_rate: %d\n" % success_rate)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--dir", type=str, default="/home/share_folder/exp")
    params = parser.parse_args()
    root_dir = params.dir
    model_name = params.model_name
    model_dir = os.path.join(root_dir, model_name)
    config_path = os.path.join(model_dir, "config_planner.yaml")
    with open(config_path, 'r') as f:
        config = yaml.load(f, Loader=yaml.FullLoader)
    
    main(config, save_dir=model_dir)