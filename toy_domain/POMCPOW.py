...
class POMCPOW(Planner):
    def __init__(self, pomdp, 
        ...
        self.num_sim_success_data = 0
        self.num_sim_fail_data = 0
    ...
    def _ActionProgWiden(self, vnode, history, state, guide_policy, k_a=1/2, alpha_a=1/2):
        _history = vnode
        if len(_history.children) <= k_a*_history.num_visits**alpha_a:
            # if self.guide == 'q':
            #     _action, _action_value, inference_time = self._pomdp.agent._policy_model.sample(history, self._pomdp.goal_state.position)
            #     if vnode[_action] is None:
            #             history_action_node = QNode(self._num_visits_init, _action_value)
            #             vnode[_action] = history_action_node
            # else:
            if guide_policy:
                # Adding stochasticity
                if _history.num_visits%4 == 3:
                    _action = self._pomdp.agent._policy_model._NextAction(history)
                else:
    ...
    # def _simulate(self, state, history, root, parent, observation, depth, logging, guide, rollout_guide, k_o=2, alpha_o=1/2): # root<-class:VNode, parent<-class:QNode
        if depth > self._max_depth:
            ...
            if save_sim_name:
                ...
                if randomize:
                    sim_data = (sim_data, self._pomdp.goal_state.position, len(self._agent.history), len(history))
                with open(os.path.join(self.save_dir_sim, 'fail', f'{save_sim_name}_{self.num_sim_fail_data}.pickle'), 'wb') as f:
                    pickle.dump(sim_data, f)
                self.num_sim_fail_data += 1
        ...
        if observation not in root[action].children:
            ...
            if self._pomdp.env.reward_model.is_goal_state(next_state):
                ...
                if guide_value == 'V':
                    total_reward = reward + self._pomdp.env.reward_model.valuenet.sample(history, self._pomdp.goal_state.position)[0]
                elif guide_value == 'Q':
                    total_reward = self._pomdp.env.reward_model.valuenet.sample(history, self._pomdp.goal_state.position, action)[0]
                else:
                    total_reward = reward
                # if self.guide == 'q':
                ...
                if save_sim_name:
                    ...
                    if randomize:
                        sim_data = (sim_data, self._pomdp.goal_state.position, len(self._agent.history), len(history))
                    with open(os.path.join(self.save_dir_sim, 'success', f'{save_sim_name}_{self.num_sim_success_data}.pickle'), 'wb') as f:
                        pickle.dump(sim_data, f)
                    self.num_sim_success_data += 1
    ...
    def _rollout(self, state, history, root, depth, logging, save_sim_name, randomize, rollout_guide=False): # root<-class:VNode
        ...
        before_rollout_history = history
        while depth < self._max_depth:
            if rollout_guide:
                action, _ = self._agent._policy_model.sample(history, self._pomdp.goal_state.position)
            else:
                action = _action = self._pomdp.agent._policy_model._NextAction(history)
            next_state, observation, reward, nsteps = sample_generative_model(self._agent, state, action)
            ...
            if self._pomdp.env.reward_model.is_goal_state(next_state):
                ...
                if save_sim_name:
                    ...
                    sim_data = np.asarray(history)
                    if randomize:
                        sim_data = (sim_data, self._pomdp.goal_state.position, len(self._agent.history), len(before_rollout_history))
                    with open(os.path.join(self.save_dir_sim, 'success', f'{save_sim_name}_{self.num_sim_success_data}.pickle'), 'wb') as f:
                        pickle.dump(sim_data, f)
                    self.num_sim_success_data += 1
