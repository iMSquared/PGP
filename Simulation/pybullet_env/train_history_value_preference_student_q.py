...
from learning.dataset.fetching_value_dataset import FetchingStudentQValueDataset
...
class Setting(Serializable):
    ...
    file_path_train_entire_annotation: str = f"/home/jiyong/vessl/{dataname}/{sim_or_exec}/train/entire.csv"
    ...
    file_path_eval_entire_annotation : str = f"/home/jiyong/vessl/{dataname}/{sim_or_exec}/eval/entire.csv"
    ...
    # evaluator  : str   = "5.9_value_pref_May2nd_cuboid12_large_q=False_sim_dataset_dim256_batch1024_lr0.0001"
    evaluator  : str   = "5.9_value_success_fail_May2nd_cuboid12_large_q=False_sim_dataset_dim256_batch1024_lr0.0001"
    temperature: float = 1.0

    # Logging 
    exp_dir       : str = '/home/jiyong/workspace/POMDP/Simulation/pybullet_env/learning/exp'
    # model_name    : str = 'test'
    # model_name    : str = f'5.9_value_pref_{dataname}_q=True_{sim_or_exec}_dim{dim_model_hidden}_batch{batch_size}_lr{learning_rate}_temp{temperature}_student'
    model_name    : str = f'5.9_value_success_fail_{dataname}_q=True_{sim_or_exec}_dim{dim_model_hidden}_batch{batch_size}_lr{learning_rate}_temp{temperature}_student'
...
def main(config: Setting):
    ...
    # Evaluator
    v_evaluator = HistoryPlaceValueonly(
    ...
    train_dataset = FetchingStudentQValueDataset(**train_dataset_kwargs)
    eval_dataset = FetchingStudentQValueDataset(**eval_dataset_kwargs)
...
class TrainWrapper:
    ...
    def train_step_fn(self, steps: int):
        ...
        init_obs_rgbd, init_obs_grasp, goal, \
            seq_action, seq_obs_rgbd, seq_obs_grasp, seq_obs_rgbd_for_label, seq_obs_grasp_for_label, \
            action_mask, obs_mask, obs_mask_for_label, \
            input_trajectory_length, full_trajectory_length \
                = ([d.to(self.config.device) for d in data])

        # Get label (V-value)
        self.v_evaluator.eval()
        with torch.no_grad():
            v_value = self.v_evaluator(
                init_obs_rgbd, init_obs_grasp, goal, 
                seq_action, seq_obs_rgbd_for_label, seq_obs_grasp_for_label,
                action_mask, obs_mask_for_label) / self.config.temperature
        ...
        # Loss calculation
        mse_loss: torch.Tensor = self.loss_fn(pred_value, v_value)
