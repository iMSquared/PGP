...
def episode(config):
    ...
    config["project_params"]["overridable"]["guide_preference"] = False
    ...
    for q in queries:
        ...
        for dyaw in dyaw_bin:
            ...
            dir_q_value = agent._value_model.sample(agent.init_observation, query_history, pomdp.env.state, agent.goal_condition)
            if dir_q_value > q_value:
                q_value = dir_q_value
        
        print(q_value)
        weights.append(q_value)
    

    print("visualizing weights with softmax temperature.")
    # weights = np_softmax(weights, temperature=0.001)
    # weights = minmax_to_01(weights)
    # 4. Draw the color
    for q, w in zip(queries, weights):
        print(w)
        pos = [q[0], q[1], 0.681]
        q_range = (max(weights) - min(weights))
        intensity = (w-min(weights)) / q_range
        ...
        bc.changeVisualShape(uid, -1, rgbaColor=[intensity, intensity, intensity, 1.0])
