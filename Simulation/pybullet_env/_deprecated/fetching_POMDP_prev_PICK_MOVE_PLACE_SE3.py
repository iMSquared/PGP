...
class FetchingObservation(Observation):
    ...
    def __init__(self, observation: Dict[int, np.ndarray], seg_mask: Dict[int, np.ndarray]):
...
class FetchingObservationModel(ObservationModel):
    ...
    def probability(self, observation: FetchingObservation, next_state: FetchingState, action: FetchingAction, segmentation=True, log=False, beta=0.1, m=5):
        # ======================== Debug =========================
        if self.config['project_params']['debug']['get_data']:
        ...
        # Reshape list into ndarray(image)
        gt_img = np.array(px_d, dtype=np.float32)
        gt_mask = np.array(px_id, dtype=np.uint8)
        
        if segmentation:
            seg_idx = np.full(gt_img.shape, False)
            # |TODO{Jiyong}|: check whether uids of state and particles are match
            for k, v in observation.seg_mask.items():
                seg_idx_gt = np.where(gt_mask==k, True, False)
                seg_idx = np.logical_or(seg_idx, v)
                seg_idx = np.logical_or(seg_idx, seg_idx_gt)
            noise = observation.observation[seg_idx] - gt_img[seg_idx]
            
        else:
            noise = observation.observation - gt_img
        log_pdf = np.log((1 - beta) * (1/np.sqrt(2*np.pi) * self.sigma) * np.exp(-np.power(noise, 2) / (2 * self.sigma**2)) + beta/m)
        log_prob = np.sum(log_pdf)
...
class FetchingRolloutPolicyModel(RolloutPolicyModel):
    ...
    def sample(self, history: Tuple, state: FetchingState, prior: List=None):
        ...
        elif type == "MOVE":   # |FIXME(Jiyong)|: hardcoded
            if self.config['project_params']['debug']['get_data']:
                debug_data['num_action'][1] += 1
            
            for i in range(self.num_filter_trials):
                # Check prior: take prior only if both uid and pose are exist
                if (prior is None) or (prior is not None and (prior[1] is None or prior[2] is None)):
                    x = random.uniform(self.config["env_params"]["binpick_env"]["cabinet"]["pos"][0] - 0.5, self.config["env_params"]["binpick_env"]["cabinet"]["pos"][0] + 0.3)
                    y = random.uniform(self.config["env_params"]["binpick_env"]["cabinet"]["pos"][1] - 0.3, self.config["env_params"]["binpick_env"]["cabinet"]["pos"][1] + 0.3)
                    z = random.uniform(self.config["env_params"]["binpick_env"]["cabinet"]["pos"][2] + 0.54, self.config["env_params"]["binpick_env"]["cabinet"]["pos"][2] + 0.69)
                    pos = (x, y, z)
