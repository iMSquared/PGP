...
class FetchingGuidedValueMSE(ValueModel):
    ...
    class Config(Serializable):
        ...
        # Experiment setup
        exp_name: str = "5.06_value_mse_May6th_3obj_depth8_3000_q=False_sim_dataset_dim256_batch1024_lr0.0001"
        weight  : str = "checkpoint6000.pth"        
        normalizer: float = 100.
    ...
    def __init__(self, bc    : BulletClient, 
        ...
        if not (config["project_params"]["overridable"]["guide_q_value"] == False \
                and config["project_params"]["overridable"]["guide_preference"] == False):
    ...
    def sample(self, init_observation: FetchingAction, 
        ...
        with torch.no_grad():
            ...
            value = pred.item() * self.nn_config.normalizer
