...
def episode(config):
    ...
    list_predq_visible_pick_target     = []
    list_predq_visible_pick_nontarget  = []
    list_predq_occluded_pick_target    = []
    list_predq_occluded_pick_nontarget = []
    count = 0
    while count < NUM_INFERENCE:
        ...
        pred_q_value = value_model.sample(agent.init_observation, inference_history, pomdp.env.state, agent.goal_condition)
        ...
        if check_visibility(agent.history[-1].observation.rgb_image, agent.history[-1].observation.seg_mask):
            if sim_env.gid_table[next_action.aimed_gid].is_target:
                list_predq_visible_pick_target.append(pred_q_value) 
            else:
                list_predq_visible_pick_nontarget.append(pred_q_value)
        else:
            if sim_env.gid_table[next_action.aimed_gid].is_target:
                list_predq_occluded_pick_target.append(pred_q_value)
            else:
                list_predq_occluded_pick_nontarget.append(pred_q_value)


    t2_occlude = len(list_predq_occluded_pick_target) \
               + len(list_predq_occluded_pick_nontarget)
    t2_visible = len(list_predq_visible_pick_target) \
               + len(list_predq_visible_pick_nontarget)

    print(f"total: {NUM_INFERENCE}, t2_occlude: {t2_occlude}, t2_visible: {t2_visible}")
    print(f"\tt2_occlude - pick_target: {len(list_predq_occluded_pick_target)}, pick_non_target: {len(list_predq_occluded_pick_nontarget)}")
    print(f"\tt2_visible - pick_target: {len(list_predq_visible_pick_target)}, pick_non_target: {len(list_predq_visible_pick_nontarget)}")


    # print    
    plt.figure(figsize=(12,6))
    plt.subplot(1, 2, 1)
    plt.boxplot([list_predq_occluded_pick_target, list_predq_occluded_pick_nontarget])
    plt.title("Occluded (t-1)")
    plt.ylabel("Q Value")
    plt.xticks([1, 2], ["PICK(target)", "PICK(non-target)"], rotation=45)
    plt.ylim(-10, 10)
    plt.subplot(1, 2, 2)
    plt.boxplot([list_predq_visible_pick_target, list_predq_visible_pick_nontarget])
