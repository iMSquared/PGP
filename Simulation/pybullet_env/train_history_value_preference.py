...
from learning.dataset.fetching_value_dataset import FetchingVValueDataset, FetchingQValueDataset, FetchingPreferenceDataset
...
class Setting(Serializable):
    ...
    # Dataset
    dataname: str = "May14th_3obj_sparse_1000"
    sim_or_exec: str = "sim_dataset"
    file_path_train_annotation        : str = f"/home/ssd2/sanghyeon/vessl/{dataname}/{sim_or_exec}/train/entire.csv"
    file_path_train_success_annotation: str = f"/home/ssd2/sanghyeon/vessl/{dataname}/{sim_or_exec}/train/success.csv"
    data_path_train_dataset_json      : str = f"/home/ssd2/sanghyeon/vessl/{dataname}/{sim_or_exec}/train/dataset_json"
    data_path_train_dataset_npz       : str = f"/home/ssd2/sanghyeon/vessl/{dataname}/{sim_or_exec}/train/dataset_numpy"
    file_path_eval_annotation         : str = f"/home/ssd2/sanghyeon/vessl/{dataname}/{sim_or_exec}/eval/entire.csv"
    file_path_eval_success_annotation : str = f"/home/ssd2/sanghyeon/vessl/{dataname}/{sim_or_exec}/eval/success.csv"
    data_path_eval_dataset_json       : str = f"/home/ssd2/sanghyeon/vessl/{dataname}/{sim_or_exec}/eval/dataset_json"
    data_path_eval_dataset_npz        : str = f"/home/ssd2/sanghyeon/vessl/{dataname}/{sim_or_exec}/eval/dataset_numpy"
    ...
    model_name      : str = f'5.14_value_pref_{dataname}_q={train_q_value}_{sim_or_exec}_dim{dim_model_hidden}_batch{batch_size}_lr{learning_rate}_norollout={no_rollout_input}_augmentsuccess={augment_success}'
...
def main(config: Setting):
    ...
    train_entire_annots       = pd.read_csv(config.file_path_train_annotation)["filename"].tolist()
    train_entire_num_subdata  = pd.read_csv(config.file_path_train_annotation)["num_subdata"].tolist()
    ...
    eval_entire_annots        = pd.read_csv(config.file_path_eval_annotation)["filename"].tolist()
    eval_entire_num_subdata   = pd.read_csv(config.file_path_eval_annotation)["num_subdata"].tolist()
    ...
    if config.train_q_value:
        print("Training Q preference...")
        success_train_dataset = FetchingQValueDataset(**success_train_dataset_kwargs)
        success_eval_dataset  = FetchingQValueDataset(**success_eval_dataset_kwargs)
        entire_train_dataset  = FetchingQValueDataset(**entire_train_dataset_kwargs)
        entire_eval_dataset   = FetchingQValueDataset(**entire_eval_dataset_kwargs)
    else:
        print("Training V preference...")
        ...
        entire_train_dataset  = FetchingVValueDataset(**entire_train_dataset_kwargs)
        entire_eval_dataset   = FetchingVValueDataset(**entire_eval_dataset_kwargs)
    preference_train_dataset = FetchingPreferenceDataset(
        success_dataset    = success_train_dataset,
        comparison_dataset = entire_train_dataset,
        augment_success    = config.augment_success )
    preference_eval_dataset = FetchingPreferenceDataset(
        success_dataset    = success_eval_dataset,
        comparison_dataset = entire_eval_dataset,
        augment_success    = config.augment_success )

    # Sampler
    train_sampler = BatchSampler(
        sampler = WeightedRandomSampler(
            weights     = preference_train_dataset.weights, 
            num_samples = len(preference_train_dataset), 
            replacement = True),
        batch_size = config.batch_size,
        drop_last = True)
    eval_sampler = BatchSampler(
        sampler = WeightedRandomSampler(
            weights     = preference_eval_dataset.weights, 
            num_samples = len(preference_eval_dataset), 
            replacement = True),
        batch_size = config.batch_size,
        drop_last = True)
