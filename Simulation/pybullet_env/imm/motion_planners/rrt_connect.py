...
from .utils import irange, RRT_ITERATIONS, INF, elapsed_time, MAX_TIME
...
def rrt_connect(start, goal, distance_fn, sample_fn, extend_fn, collision_fn,
                max_iterations=RRT_ITERATIONS, max_time=MAX_TIME, **kwargs):
    """
...
def birrt(start, goal, distance_fn, sample_fn, extend_fn, collision_fn, max_solutions=5, **kwargs):
    """
    :param start: Start configuration - conf
    :param goal: End configuration - conf
    :param distance_fn: Distance function - distance_fn(q1, q2)->float
